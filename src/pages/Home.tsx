      ...
      <section id="about" className="section-spacing content-padding">
        <div className="max-w-4xl mx-auto">
          ...
          <ScrollReveal delay={0.1}>
            <article className="mb-10 border-b border-border pb-10">
            ...
            <article>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {[
                  ...
                  <div key={item.title} className="space-y-2 border-b border-border pb-4">
            ...
            {(isDark ? trustSignalsDark : trustSignalsLight).map((item, i) => (
              <ScrollReveal key={item.title} delay={i * 0.1}>
                <article className="space-y-3">
                  <img src={item.image} alt={item.title} className="w-full aspect-square object-cover rounded-2xl" loading="lazy" />
                  <div className="flex items-center gap-2">
                    <item.icon className="w-4 h-4 icon-gradient-cyan" />
        ...
        <div className="max-w-6xl mx-auto">
          <ScrollReveal>
            ...
            <p className="text-center text-muted-foreground mb-16 max-w-xl mx-auto">
              Aşağıdaki tüm alanlarda ve daha birçok farklı hukuk alanında profesyonel danışmanlık hizmeti sunulmaktadır.
            </p>
          </ScrollReveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {featuredAreas.map((area, i) => (
              <ScrollReveal key={area.id} delay={i * 0.08}>
                <article className="space-y-3">
                  <img src={(isDark ? areaImagesDark : areaImagesLight)[area.id] || ''} alt={area.title} className="w-full aspect-[4/3] object-cover rounded-2xl" loading="lazy" />
          ...
          <ScrollReveal delay={0.1}>
            <div className="max-w-lg mx-auto">
              {[
                ...
                <article key={item.label} className="flex items-center justify-between border-b border-border py-4">
